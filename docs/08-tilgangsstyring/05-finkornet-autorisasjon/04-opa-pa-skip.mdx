...
Hvis du har kommet hit har du skrevet OPA-policyer og gjort de nødvendige endringene i applikasjonen din til å kalle OPA ved autorisasjonssjekker.
Neste steg er å bygge en OPA bundle og gjøre den tilgjengelig for OPA-sidecaren. En *bundle* er en gzippet
tarball som pakker kompilerte Rego-policyer, statiske JSON-data og en `.manifest`-fil, og er OPAs distribusjonsenhet som en
kjørende OPA-server kan polle etter og laste inn uten restart.

## Bygge bundle i GitHub Actions

Team Tilgangsstyring har lagd workflowen `kartverket/github-workflows/.github/workflows/build-and-push-opa-rules.yml`, som gjør det enkelt for
deg å bygge og publisere OPA-bundler til GHCR i GitHub Actions.

### Eksemepel på bruk av workflow
...
jobs:
  build:
    uses: kartverket/github-workflows/.github/workflows/build-and-push-opa-rules.yml@<commit-sha>
    with:
...
DU har nå bygd en OPA bundle og publisert den til GHCR, og er klar til å bruke den i applikasjonen din. For å bruke OPA i applikasjonen din så trenger du
å få OPA injisert som en sidecar sammen med din applikasjon. Dette gjøres ved å [spesifisere OPA som tilgjengelig sidecar-tjeneste](../09-accesserator/index.mdx#-spesifisere-tilgjengelige-sidecar-tjenester-for-din-skiperator-applikasjon).
Deretter kan du konfigurer hvilke bundles OPA skal benytte under autorisajon med en `SecurityConfig`-ressurs.

### Eksempel på `SecurityConfig`

Følgende `SecurityConfig` konfigurerer OPA-sidecaren til å laste inn *to* bundles.
- En bundle bestående av autorisasjonsregelene bygd fra mappen `.opa/policy`
- En bundle bestående av produksjonsspesifikke data som reglene dine bruker, bygd fra mappen `.opa/data/prod`
  ...
  opa:
    ...
    bundleUrls:
      ...
      - name: group_access_data
        url: ghcr.io/kartverket/<REPO>/opa-data:<TAG>@sha256:<DIGEST>
        verification:
          source:
            repository: kartverket/<REPO>
            workflow: .github/workflows/build-opa-bundle.yml
...
OPA er tilgjengelig via miljøvariabelen `OPA_URL` eller direkte på `http://localhost:3010`.
Applikasjonen din en nå klar til å kalle OPA for autorisasjonssjekker.
