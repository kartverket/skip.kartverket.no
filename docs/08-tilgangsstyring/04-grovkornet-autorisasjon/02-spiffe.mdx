...
Ztoperator baserer seg, i likhet med SPIFFE, på å opprette `AuthorizationPolicy`-ressurser. Det er derfor viktig å være
klar over at Ztoperator og egendefinerte SPIFFE-baserte `AuthorizationPolicy`-ressurser kan komme i konflikt. Se
...
Istio-proxyen foran din applikasjon setter headeren `X-Forwarded-Client-Cert` på innkommende forespørseler. Denne
headeren inneholder informasjon om SPIFFE-identiteten til avsender og mottaker av forespørselen, og har følgende format:

```
By=spiffe://cluster.local/ns/<mottakers namesapce>/sa/<mottakers applikasjon>;Hash=<hash>;Subject="";URI=spiffe://cluster.local/ns/<avsenders namesapce>/sa/<avsenders applikasjon>
```

Det mest naturlige er da å bruke URI-delen av headeren for å validere SPIFFE-identiteten. Applikasjonen kan alltid stole
på at denne headeren er korrekt, og at den ikke kan forfalskes av klienten.

:::info
Når trafikken kommer fra internett, dvs. via en ingress, vil avsender alltid være Istio Ingress Gateway, ettersom det er
denne komponenten som er ansvarlig for å rute trafikk fra internett til riktig applikasjon i clusteret.

Eksempel: `URI=spiffe://cluster.local/ns/istio-gateways/sa/istio-external`
:::


## Testing av SPIFFE-regler
SPIFFE-regler håndheves på SKIP med Kubernetes-ressursen `AuthorizationPolicy`. For å teste denne ressursen kreves det
et kjørende Kubernetes-cluster med Istio og SPIFFE-sertifikater. Det er derfor vanskelig å teste dette på en
applikasjonsnær måte lokalt og i CI/CD.
