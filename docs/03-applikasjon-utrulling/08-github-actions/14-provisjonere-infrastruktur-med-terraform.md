...
Hvilke roller denne deploykontoen trenger avhenger av hvilke ressurser man ønsker å opprette, ofte vil de være på admin-nivå - typiske roller man trenger vil være:
* Oppretting av bøtter - `roles/storage.admin`
* Administere rettigheter til andre brukere og servicekontoer - `roles/iam.securityAdmin`
* Administrering av WIF - `roles/iam.workloadIdentityPoolAdmin`
* Impersonate for WIF - `roles/iam.workloadIdentityUser`
...
Hvis du trenger å teste Terraform-koden ved å kjøre `terraform plan` lokalt, vil dette i utgangspunktet feile fordi du kjører som egen bruker i stedet for deploykontoen.
...
For eksempler på hvordan dette kan settes opp med tanke på opprettelse av blant annet lagringsbøtter i GCP, se vårt [terraform-modules](https://github.com/kartverket/terraform-modules/tree/main/cloud_storage) repo på GitHub - modulen cloud_storage inneholder noen gode eksempler og kan også brukes i egen kildekode som modul.
