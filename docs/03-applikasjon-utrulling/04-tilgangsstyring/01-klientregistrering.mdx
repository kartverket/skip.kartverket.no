...
Man kan benytte seg av [Skiperator](../03-skiperator/04-api-docs.md) dersom klientregistreringen hører til en SKIP-applikasjon.
...
<Tabs queryString={"operator"}>
    <TabItem value="skiperator" label="Skiperator">
        ...
        Skiperator tilbyr et forenklet API i CRD-en `Application` for å registrere klienter mot **ID-porten**, **Maskinporten** og **Entra ID**.
        ...
        <Tabs queryString={"idp"}>
            <TabItem value="entraid" label="Entra ID">
                ...
                Følgende eksempel oppretter en app-registrering i Entra ID.
                - Kun direkte medlemmer av gruppene `42acac87-d6e2-4d1c-ab7a-b8634f68fb6e` og `f749a88e-0811-4b73-95d4-18e7d8e348c9`
                vil få tilgang til applikasjonen og gruppe ID-ene vil også inkluderes i `groups`-claimet i tokenet.
                - App-registreringen `other-cluster:other-namespace-other-app` vil bli lagt til som en forhåndsautorisert app og få
                tildelte app-rollen `app_role` og scopet `custom_scope`.
                - Ettersom redirect URL-er ikke er eksplisitt spesifisert vil `https://an-ingress.com/oauth2/callback` og
                `https://another-ingress.com/oauth2/callback` blir registrert som redirect URI-er.
                - Ettersom logout URL ikke er eksplisitt spesifisert vil den første ingressen bli registrert som logout URL, i.e.
                `https://an-ingress.com/oauth2/logout`.
...
// diff-add-start
  entraid:
    ...
    claims:
      groups:
        - id: 42acac87-d6e2-4d1c-ab7a-b8634f68fb6e
        - id: f749a88e-0811-4b73-95d4-18e7d8e348c9
...
```
            ...
            <TabItem value="idporten" label="ID-porten">
                ...
                Følgende eksempel konfigurerer en integrasjon mot ID-porten.
...
```
            ...
            <TabItem value="maskinporten" label="Maskinporten">
                ...
                via spesifikasjonen til `Application`.
                Følgende eksempel konfigurerer en integrasjon mot Maskinporten og eksponerer scopet `innsyn` og setter HUSLØS HURTIG TIGER AS og KOSTBAR LEKKER APE AS
                som godkjente konsumenter av dette scopet.
                Resten av feltene som støttes kan du finne i Skiperator sin [API-dokumentasjon](../03-skiperator/04-api-docs.md).
...
```
    ...
    <TabItem value="native" label="Digdirator / Azurerator">
        ...
        Hvis man ønsker å registrere en klient fritt uten å knytte den til en Skiperator-applikasjon, kan man benytte seg av CRD-ene som Digdirator og Azurerator introduserer direkte.
        Dette gjør det mulig å definere klientregistreringen uavhengig av en spesifikk applikasjon i Skiperator.
