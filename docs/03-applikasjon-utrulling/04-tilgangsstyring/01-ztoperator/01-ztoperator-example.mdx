...
og det er derfor viktig at man spesifiserer samme rotsti som den konfigurerte [`post_logout_redirect_url`](https://openid.net/specs/openid-connect-rpinitiated-1_0.html) i klientregistreringen. For identitetstilbydere som tillater flere brukere per person, som Microsoft Entra ID, er det viktig å logge seg ut med samme bruker man brukte for å logge inn ettersom Ztoperator legger til `id_token_hint` i logout-forespørselen. 
