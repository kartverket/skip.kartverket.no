...
- En Ztoperator-`AuthPolicy` som håndhever at angitte endepunkter **må** presentere et gyldig `access_token`, og i tillegg logger inn brukere der det er angitt.

Manifestene for disse ressursene finner du i [tilgangsstyring-apps på GitHub](https://github.com/kartverket/tilgangsstyring-apps/tree/main/env/atgcp1-sandbox/ztoperator-demo).
...
:::note
Redirect-url er noe `demo-app` ikke trenger å forholde seg til. Ztoperator-`AuthPolicy` konfigures med en redirect-url når man konfigurerer innlogging av brukere, og håndterer [callbacks i authorization code flow](https://datatracker.ietf.org/doc/html/rfc6749#section-1.3.1).
:::
...
`spec.wellKnownURI` trengs for å utføre [OpenID Connect Discovery](https://swagger.io/docs/specification/v3_0/authentication/openid-connect-discovery/) og hente ut relevant informasjon om identitetstilbyderren brukt under autentisering. Det er her man knytter Ztoperator-`AuthPolicy` til en identitetstilbyder.
...
- `spec.autoLogin.loginPath` spesifiserer en dedikert sti man kan aksessere for å logge inn en bruker. Ztoperator tar seg av selve autentiseringen og kaller stien i applikasjonen din etterpå, så implementer denne stien for å sende brukeren videre slik du ønsker.
- `spec.autoLogin.logoutPath` spesifiserer hvilken sti man skal aksessere for å logge ut brukeren lokalt og opp mot den konfigurerte identitetstilbyderen ([RP-initiated logout](https://openid.net/specs/openid-connect-rpinitiated-1_0.html)). Etter vellykket utlogging vil brukeren bli omdirigert til rotstien til applikasjonen, dvs. `https://<INGRESS>/`.
For identitetstilbydere som tillater flere brukere per person, som Microsoft Entra ID, er det viktig å logge seg ut med samme bruker man brukte for å logge inn. 
Dette er ikke et problem for identitetstilbydere som ID-porten, som kun tillater én bruker per person.
- `spec.autoLogin.redirectPath` spesifiserer omdirigeringssti brukt under _Authorization Code Flow_. Dette er en sti som du **ikke** tregner å håndtere i applikasjonskoden din, men den må samsvare med en av de konfigurerte omdirigeringsstiene i klientregistreringen din.
...
:::tip
Ztoperator benytter seg av cookies for å lagre innlogget sesjon i nettleseren. Disse cookiene er HTTP-only og kan derfor ikke aksesseres av JavaScript i nettleseren. De er i tillegg kryptert og det er kun Ztoperator som kan dekryptere dem og sende dem trygt videre nedstrøms til den beskyttede applikasjonen.
:::

:::warning
Ztoperator-`AuthPolicy` benytter seg av cookies for å lagre innlogget sesjon i nettleseren. [OIDC Single Logout (SLO)](https://docs.digdir.no/docs/idporten/oidc/oidc_func_sso.html#single-logout-slo) benytter seg av `iframe` for å logge brukeren ut av alle nettstedene som brukeren har en aktiv SSO-sesjon hos. Ettersom cookies som Ztoperator-`AuthPolicy` setter er HTTP-only så vil de ikke kunne aksesseres i en `iframe`, og SLO er derfor ikke støttet av Ztoperator-`AuthPolicy`.
:::
...
[Både ID-porten og Maksinporten benytter seg av audience-begrensing](https://docs.digdir.no/docs/idporten/oidc/oidc_func_aud.html), en mekanisme basert på [RFC 8707 Resource Indicators for Oauth2](https://tools.ietf.org/html/rfc8707), som motvirker at tokens kan misbrukes mot andre APIer enn de som er tiltenkt. En konsekvens av dette er at det er klienten sitt ansvar å be om et audience-begrenset token. Dette er støttet i Ztoperator-`AuthPolicy` med feltet `spec.acceptedResources`. Dette feltet spesifiserer hvilke APIer tokenet skal kunne brukes mot.
...
:::caution
[Klientregistrering med Digdirator](../klientregistrering#-digdirator) antar at klienten _kun_ benytter seg av `private_key_jwt` som klient-autentiseringsmetode, noe som gjør at klientintegrasjoner satt opp med Digdirator per nå ikke er kompatibelt med Ztoperator-`AuthPolicy`.
:::
