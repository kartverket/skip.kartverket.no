...
<Tabs groupId='operator'>
    <TabItem value='ztoperator' label='Ztoperator'>
        ...
        <Admonition type="warning" title="Husk...">
            Ved å navigere til stien som settes som utloggingssti (`logoutPath`), termineres den innloggede sesjonen både lokalt **og** hos den konfigurerte identitetsleverandøren (IDP).
            Ztoperator vil deretter navigere brukeren tilbake til rot (`https://<ingress>/`). Dette **må** samsvare med det som er konfigurert som `post_logout_redirect_uri` hos IDP for at utlogging hos IDP skal fungere.
        </Admonition>
