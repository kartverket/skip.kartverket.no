...
understanding of cloud technologies and the surrounding legal aspects. This led to a fear of the unknown, and
...
As a platform team, our job is to provide the product teams with a platform that
is as easy to use as possible. With the growing complexity of the stack, especially
when you add hybrid to the mix, you start getting to the point where the
complexity becomes unmanageable for the product teams. This is where our role
as platform engineers come in. We build the right abstractions so that the
complexity of the platform is hidden from the product teams.
...
canary-testing the migration of some of our larger applications to SKIP.

And don't let the product teams configure the service mesh directly using service
mesh resources. This is a recipe for disaster. We tried this in the beginning,
and first of all it's a huge complexity burden for the product teams, but we
also started getting a lot of weird issues when product teams would configure
the mesh in ways that broke their encapsulation. Since the service mesh is a
cluster-wide feature, if one team makes an invalid configuration, it can break
other teams' workloads. Kubernetes namespaces be damned. We've therefore moved
to a model where the platform team provides an abstraction through
[Skiperator](https://github.dev/kartverket/skiperator) which configures the
service mesh for them.
...
If you need service mesh them limit the scope until you get comfortable with the
service mesh, for example start with just mTLS and observability, and then add
zero trust networking features later.
...
some consultants to work on the platform. Most of these engineers some
...
configuration managment. Managed services do come with some tradeoffs, however,
...
over our platform. This of course has the downside of having to maintain these
...
regret using a pricy managed offering in the beginning. It helped us get started
