...
of these features, like EnvoyFilters, are supported by Anthos Service Mesh,
which is why we're exploring using upstream Istio instead of ASM.

One thing we quickly learned is don't let the product teams configure the
...
Managed services are, however, opinionated, so if you have specific
requirements, you may have to work around them.
