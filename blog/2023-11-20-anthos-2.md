...
While doing the install we started when Anthos was very new, and did not really 
have many options for architecture. We wanted to use existing underlying vmware 
infrastructure, so the nodes in our clusters are VMs, provisioned by an install
script provided by Google. Our cluster is installed with [kubeception](https://kubernetes.io/blog/2017/01/how-we-run-kubernetes-in-kubernetes-kubeception/), this is
no longer recommended. The recommended model called [Controlplane V2](https://cloud.google.com/anthos/clusters/docs/on-prem/latest/how-to/create-user-cluster-controlplane-v2)
