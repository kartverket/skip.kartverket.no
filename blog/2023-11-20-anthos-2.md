...
### Service Mesh
A [Service Mesh](https://istio.io/latest/about/service-mesh/) in Kubernetes is an infrastructure layer that manages communication between services. We are using Anthos Service Mesh (ASM), this is based on [Istio](https://istio.io) and nicely integrated with the gcp console. It is easy to get an overview of services, the connection between them, and what services are connected to either our internal or external gateways. This can be displayed in a Topology view, or if you click a service, you get a more detailed list.

![Anthos Service Mesh](img/services.png)
A snippet of services running in our sandbox cluster.

When we deploy services to our cluster we create almost all kubernetes and service-mesh resources with [Skiperator](https://github.com/kartverket/skiperator). This operator configures the resources to fit our setup, and makes "best practice" the easy way. This has been one of the great success stories in SKIP, and Skiperator is in continuos development.
...
ASM is as previously mentioned based on Istio, and after some internal discussion we decided to experiment with running upstream Istio in our GKE clusters in GCP. Pairing it with ASM in our on-premise clusters worked as expected, and we are now running upstream Istio, with ASM in a multi-cluster setup. We also looked into using managed ASM in our GKE cluster, this was hard for us however, since it required firewall openings for sources we could not predict.

![Multi-Primary on different networks](img/multi-cluster.png)
We have chosen the [Multi-Primary on different networks](https://istio.io/latest/docs/setup/install/multicluster/multi-primary_multi-network/) after reviewing our network config. We connect our on-premise network, with the GCP VPC through a VPN connection. To have a production ready environment, the VPN connection must be configured with redundancy, and 
